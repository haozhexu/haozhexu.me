...
Since 2014 when I first started photography with a proper camera, so far I've used a range of cameras and lenses, I've been chasing the latest and greatest gears and also jumped between different combinations of cameras and lenses for street photography. While I enjoyed trying different equipments, I always find different issues with different cameras for street photography. It's difficult to design a modern camera and it's almost impossible to get it right for different photography purposes, so we can't fail a camera simply because it's not great for street photography. Still, there's something missing for me, and thus I came up with the idea of making a camera app for street photography.

Compared to dedicated cameras, smartphone cameras have two major advantages:

1. Smartphones are more socially acceptable. We tried to use the best camera for street photography, for example, the ones with leaf shutter for quiet operation, and the ones that are small enough to not look like a professional camera, but nonetheless, when you hold a camera and try to get close to people, they still recognize that as a camera and interpret that as you would take a photo of them using that camera for some purposes.
...
4. **Lenses**: lenses are fixed, even if there are multiple lenses.
...
Before going to the details, let me first explain its name. I'm currently located in Melbourne, which is a city full of cultures from different countries around the world, "Mo" is the first two letters from the Chinese pronunciation of Melbourne, and "Ca" is the first two letters of the word "camera", now you get Moca. As a bonus, Moca sounds the same as mocha, which also echos the coffee culture of Melbourne.

## Control

The most important button for a camera is probably the shutter button, which is even the only button on certain old cameras. Smartphones have limited physical buttons, the home button, volume adjustment buttons, and the button to switch the phone on and off. Most user activities happen on the display with virtual controls. Some devices let you use volume buttons to take photo, the problem with this is the volume buttons are not tuned as a shutter button, and people like to have protection case for their phones which either cover the volume buttons with rubber or expose the volume buttons but make them less protruding, in either case, the volume buttons become less easy to press and pressing them can easily introduce a shake that makes the photo blurred.
...
1. Detect the gesture and check what function it's assigned to, and do nothing, or:
2. Do not detect the gesture at all if the function is disabled

Moca takes the second approach, the reason is always for the purpose of taking the decisive moment, have multiple gesture detection with some of them doing nothing can create lag compared to only detect the gestures that do have functions assigned, for example, we have single tap to capture photo, if we have long press function enabled, when you have your finger on the screen, the system has to wait a little bit in order to figure out if you are doing a single tap, or a long press; Similarly, if you have swipe gesture enabled, the system has to also figure out if a touch ends up being a quick tap, or the first step of a swipe. I've used X-E3 and X100V from Fujifilm, both of them have touch screen functions but none of them is smooth enough, someone [reported this on DPReview](https://www.dpreview.com/forums/post/64591131) and there's a reply suggesting disabling the swipe function can make the touch function more smooth, which aligns with my theory that multiple gesture detections can make the operation laggy.
...
Due to the small sensor, there's no good way to resolve low light noise - if you don't want to reply on computational technology, battery life is also a problem, I'll think more about it for the next version update. Lenses are not really a problem even though iPhones have fixed camera, street photographers prefer prime lense and all iPhones should have at least the wide angle camera which is fine for street photography, some models even have telephotos and super wide angle cameras.

## Free is Free

The last thing and probably the most important thing for your wallet, is the app is free on AppStore, free means free, not the kind of free where you can download but have to watch ads, which is also against our principle of simplicity. If you do want to support me, thank you, please give me a good rating on AppStore.
